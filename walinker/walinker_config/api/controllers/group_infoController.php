...
require_once __DIR__ . '/../firebase/user_firebase.php'; // 🔔 Notification support
...
function handle_group_info_request($route_parts, $conn) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        try {
            ...
            // 🔔 Push notification
            $userQuery = $conn->prepare("SELECT user_id FROM `group` WHERE group_id = ?");
            $userQuery->bind_param("i", $group_id);
            $userQuery->execute();
            ...
            if ($userRow = $userResult->fetch_assoc()) {
                ...
                if ($user_id) {
                    sendPushNotification(
                        ...
                        $user_id,
                        "Group Approved ✅",
                        "Your group '{$group_name}' has been approved!",
                        "Main" // MainActivity open
                    );
                }
            }
