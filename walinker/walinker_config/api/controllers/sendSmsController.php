...
function handle_send_sms_request($route_parts, $conn) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($res['success'])) {
            ...
            // ===============================
            // ✅ নতুন অংশ: POST সফল হলে পুশ নোটিফিকেশন পাঠানো
            // ===============================
            $firebaseFile = realpath(__DIR__ . '/../firebase/user_firebase.php');
            if ($firebaseFile && file_exists($firebaseFile)) {
                require_once $firebaseFile;
                onSendSmsPost($conn, [
                    "user_id" => $insert_user_id,
                    "sms" => $sms
                ]);
            } else {
                error_log("❌ Firebase file not found: " . $firebaseFile);
            }
